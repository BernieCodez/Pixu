...
class AnimationManager {
  ...
  validateAndCopyPixels(pixels, width, height) {
    ...
    if (pixels.length !== height) {
      console.warn(
        `Pixel height mismatch: expected ${height}, got ${pixels.length}`
      );
      return this.createEmptyPixelArray(width, height);
    }
  ...
  initializeSpriteFrames(sprite) {
    if (!sprite.frames || sprite.frames.length === 0) {
      ...
      if (sprite.layers && Array.isArray(sprite.layers)) {
        ...
        sprite.getLayersData &&
        typeof sprite.getLayersData === "function"
      ) {
  ...
  addFrame(insertAfter = null) {
    ...
    // Insert frame
    const insertIndex =
      insertAfter !== null ? insertAfter + 1 : sprite.frames.length;
  ...
  deleteFrame(frameIndex = null) {
    ...
    const indexToDelete =
      frameIndex !== null ? frameIndex : this.currentFrameIndex;
  ...
  moveFrame(fromIndex, toIndex) {
    ...
    if (
      fromIndex < 0 ||
      fromIndex >= sprite.frames.length ||
      toIndex < 0 ||
      toIndex >= sprite.frames.length ||
      fromIndex === toIndex
    ) {
      return false;
    }
  ...
  play() {
    ...
    this.animationInterval = setInterval(() => {
      ...
      if (this.playbackMode === "loop") {
        nextFrame =
          (this.currentFrameIndex + this.direction + sprite.frames.length) %
