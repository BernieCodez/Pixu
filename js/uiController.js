...
class UIController {
    constructor(editor) {
        ...
        this.colorPalette = [
            ...
            '#ffa500', '#a52a2a', '#dda0dd', '#98fb98', '#f0e68c', '#deb887', '#5f9ea0', '#ff1493',
            '#00bfff', '#32cd32', '#ffb6c1', '#87ceeb', '#daa520', '#b22222', '#20b2aa', '#f08080'
        ];
    ...
    initializeColorWheel() {
        ...
        if (colorWheelContainer && window.iro) {
            this.colorPicker = new iro.ColorPicker("#color-wheel", {
                width: 120,
                color: {
                    hsl: { h: 0, s: 50, l: 50 }  // Start in the middle of the color spectrum
                },
                borderWidth: 2,
                borderColor: "#333"
            });
    ...
    duplicateCurrentLayer() {
        if (this.editor.layerManager) {
            try {
                ...
                if (newLayer) {
                    this.updateLayersList();
                    this.showNotification(`Duplicated layer: ${newLayer.name}`, 'success');
                } else {
        ...
        if (this.editor.layerManager) {
            ...
            for (let i = layers.length - 1; i >= 0; i--) {
                ...
                layerItem.innerHTML = `
                    ...
                    <div class="layer-opacity" style="font-size: 12px; color: #999; margin-left: 8px;">
            ...
            if (item.type === 'separator') {
                ...
                if (!item.disabled) {
                    menuItem.addEventListener('mouseenter', () => {
                        menuItem.style.backgroundColor = item.danger ? 'rgba(255, 107, 107, 0.1)' : 'rgba(255, 255, 255, 0.1)';
                    });
    ...
    applyResize() {
        ...
        if (width > 0 && height > 0 && width <= 256 && height <= 256) {
            ...
            this.showNotification('Invalid dimensions. Use values between 1 and 256.', 'error');
        }
    }
    ...
    handleContextMenuOutsideClick(event) {
        if (this.activeContextMenu && !this.activeContextMenu.contains(event.target)) {
            this.hideContextMenu();
        }
    }
